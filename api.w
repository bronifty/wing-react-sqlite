...
// api.get("/increment", inflight (request: cloud.ApiRequest): cloud.ApiResponse => {
//     let newCount = counter.inc();
...
//     "/": "<button onclick='incrementCount()'>Increment</button><p id='count'>0</p><script>async function incrementCount() { const response = await fetch('/increment'); const newCount = await response.text(); document.getElementById('count').innerText = newCount; }</script>"
// });
...
// https://0ngxk14w3i.execute-api.us-east-1.amazonaws.com/prod/orders
// https://0ngxk14w3i.execute-api.us-east-1.amazonaws.com/prod